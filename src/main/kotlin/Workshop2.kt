...
fun main() {
    ...
    val totalElecPriceOver500Sequence = products
        ...
        .filter { it.category == "Electronics" }   // Intermediate operation (lazy)
        .filter { it.price > 500 }                 // Intermediate operation (lazy)
        .map { it.price }                          // Intermediate operation (lazy)
        .sum()                                     // Terminal operation -> เริ่มประมวลผลจริงตรงนี้
